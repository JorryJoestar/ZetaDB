...
func main() {
	...
	p66.InsertTuple(CreateNewTuple("Morries", 66, 0, schema))
	p66.InsertTuple(CreateNewTuple("simeon", 66, 1, schema))
	p66.InsertTuple(CreateNewTuple("Jorry", 66, 2, schema))
	...
	setDifferenceIt := execution.NewSetDifferenceIterator()
	setDifferenceIt.Open(seqIt72, seqIt66)

	for setDifferenceIt.HasNext() {
		tup, _ := setDifferenceIt.GetNext()
